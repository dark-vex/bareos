...
do not point users at upstream `barcus/bareos` as an alternative for v22:
its own source tree tops out at v21 and its Docker Hub
`22-alpine`/`22-alpine-pgsql` tags haven't been pushed in over a year, with
no `22-ubuntu` variant ever published there.
